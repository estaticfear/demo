class Cmat {
    constructor() {
        ...
        Cmat.initResources();
        Cmat.handleCounterUp();
        Cmat.initMediaIntegrate();
        if (CmatVariables && CmatVariables.authorized === '0') {
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = CmatVariables.languages.notices_msg.error;
                    break;
                case 'success':
                    messageHeader = CmatVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Cmat.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Cmat.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Cmat.showError(data.responseJSON.message);
                } else {
                ...
                Cmat.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + CmatVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        Cmat.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Cmat.callScroll($('.list-item-checkbox'));
        }
    ...
    static initMediaIntegrate() {
        ...
        if (jQuery().rvMedia) {
            ...
            Cmat.gallerySelectImageTemplate = `<div class="list-photo-hover-overlay">
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Cmat.gallerySelectImageTemplate;
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Cmat.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Cmat.onDragTool);
            $(window).on('mouseup', Cmat.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Cmat.onDragTool);
        $(window).off('mouseup', Cmat.onReleaseTool);
    }
