    ...
    <div class="max-width-1200" id="main-order-content">
        <div class="ui-layout">
            <div class="flexbox-layout-sections">
                @if ($order->status == \Cmat\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                ...
                <div class="flexbox-layout-section-primary mt20">
                    <div class="ui-layout__item">
                        <div class="wrapper-content">
                                        ...
                                        <div class="table-wrap">
                                            <table class="table-normal table-none-border table-color-gray-text">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end color-subtext pl10">
                                                        @if (is_plugin_active('payment') && $order->payment->id)
                                                            <a href="{{ route('payment.show', $order->payment->id) }}"
                                                               target="_blank">
                                                                <span>{{ format_price($order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::COMPLETED ? $order->payment->amount : 0) }}</span>
                                                            </a>
                                                        @else
                                                            <span>{{ format_price(is_plugin_active('payment') && $order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::COMPLETED ? $order->payment->amount : 0) }}</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                                @if (is_plugin_active('payment') && $order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::REFUNDED)
                                                    <tr class="hidden">
                                                ...
                                                <tr class="hidden">
                                                    ...
                                                    <td class="text-end pl10">
                                                        <span>{{ format_price(is_plugin_active('payment') && $order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::COMPLETED ? $order->amount : 0) }}</span>
                                                    </td>
                                                </tr>
                                                </tbody>
                                            </table>
                            ...
                            @if ($order->status != \Cmat\Ecommerce\Enums\OrderStatusEnum::CANCELED || $order->is_confirmed)
                            ...
                            <div class="pd-all-20 border-top-title-main">
                                <div class="flexbox-grid-default flexbox-flex-wrap flexbox-align-items-center">
                                    ...
                                    @elseif (is_plugin_active('payment') && $order->payment->id)
                                        <div class="flexbox-auto-left">
                                            @if (!$order->payment->status || $order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::PENDING)
                                                <svg class="svg-next-icon svg-next-icon-size-24 svg-next-icon-gray">
                                                    <use xmlns:xlink="http://www.w3.org/1999/xlink"
                                                         xlink:href="#next-credit-card"></use>
                                                </svg>
                                            @elseif ($order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::COMPLETED || $order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::PENDING)
                                        ...
                                        <div class="flexbox-auto-content ml15 mr15 text-upper">
                                            ...
                                            @elseif ($order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::COMPLETED)
                                        ...
                                        @if (!$order->payment->status || in_array($order->payment->status, [\Cmat\Payment\Enums\PaymentStatusEnum::PENDING]))
                                            <div class="flexbox-auto-left">
                                        ...
                                        @endif
                                        @if ($order->payment->status == \Cmat\Payment\Enums\PaymentStatusEnum::COMPLETED && (($order->payment->amount - $order->payment->refunded_amount > 0) || ($order->products->sum('qty') - $order->products->sum('restock_quantity') > 0)))
                                            <div class="flexbox-auto-left">
                                                <button
                            ...
                            @if (EcommerceHelper::countDigitalProducts($order->products) != $order->products->count())
                                <div class="pd-all-20 border-top-title-main">
                                    <div class="flexbox-grid-default flexbox-flex-wrap flexbox-align-items-center">
                                        @if ($order->status == \Cmat\Ecommerce\Enums\OrderStatusEnum::CANCELED && !$order->shipment->id)
                                            <div class="flexbox-auto-left">
                                                <svg class="svg-next-icon svg-next-icon-size-20 svg-next-icon-green">
                                                    <use xmlns:xlink="http://www.w3.org/1999/xlink"
                                                         xlink:href="#next-checkmark"></use>
