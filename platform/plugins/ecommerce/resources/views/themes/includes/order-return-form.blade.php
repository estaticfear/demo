    ...
    {!! Form::open(['url' => route('customer.order_returns.send_request'), 'method' => 'POST']) !!}
        ...
        @if (! EcommerceHelper::allowPartialReturn())
            <div class="col-sm-6 col-md-3 form-group">
                ...
                {!! Form::select('reason', \Cmat\Ecommerce\Enums\OrderReturnReasonEnum::labels(), old('reason'), [
                            ...
                            @foreach ($order->products as $key => $orderProduct)
                                <tr>
                                    ...
                                    @if (EcommerceHelper::allowPartialReturn())
                                        ...
                                        <td class="text-center">
                                            {!! Form::select('return_items[' . $key . '][reason]', \Cmat\Ecommerce\Enums\OrderReturnReasonEnum::labels(), old('return_items.' . $key . '.reason'), [
                                                'class' => 'form-control form-select',
                                                'placeholder' => __('Choose Reason'),
                                            ]) !!}
                                        </td>
