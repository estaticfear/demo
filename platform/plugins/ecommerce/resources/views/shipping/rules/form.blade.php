            ...
            <div class="flexbox-grid-default">
                <div class="flexbox-content-no-padding">
                    <div class="mb-3">
                        ...
                        {!! Form::customSelect('type', ['' => trans('plugins/ecommerce::shipping.rule.select_type')] + \Cmat\Ecommerce\Enums\ShippingRuleTypeEnum::availableLabels($rule ? $rule->shipping : null), $rule ? $rule->type : '', ['class' => 'select-rule-type'], \Cmat\Ecommerce\Enums\ShippingRuleTypeEnum::toSelectAttributes()) !!}
                    </div>
                </div>
                <div class="flexbox-content-no-padding pl15 rule-from-to-inputs @if ($rule && ! $rule->type->showFromToInputs()) d-none @endif">
                    <div class="mb-3">
                        <label class="text-title-field rule-from-to-label">
                            {{ $rule ? $rule->type->label() : \Cmat\Ecommerce\Enums\ShippingRuleTypeEnum::BASED_ON_PRICE()->label() }}
                        </label>
                        <div class="flexbox-grid-default flexbox-align-items-center">
                            <div class="flexbox-auto-content">
                                <div class="next-input--stylized">
                                    <span class="next-input-add-on next-input__add-on--before unit-item-label">
                                        {{ $rule ? $rule->type->toUnit() : \Cmat\Ecommerce\Enums\ShippingRuleTypeEnum::BASED_ON_PRICE()->toUnit() }}
                                    </span>
