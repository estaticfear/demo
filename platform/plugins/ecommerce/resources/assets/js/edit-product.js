class EcommerceProduct {
    ...
    handleEvents() {
        ...
        $(document).on(
            ...
            (event) => {
                ...
                if (ids.length === 0) {
                    Cmat.showError(
                        CmatVariables.languages.tables.please_select_record
                    );
                    return false;
                }

                $("#delete-selected-variations-button").data(
                    "href",
                    $current.data("target")
                );
    ...
    handleAddVariations() {
            ...
            $.ajax({
                ...
                success: (res) => {
                    if (res.error) {
                        ...
                        $("#product-variations-wrapper").load(
                            window.location.href +
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        if (res.error) {
                            ...
                            _self.initElements();
                            Cmat.initResources();
                            $("#store-product-variation-button").data(
                                "target",
                                $current.data("target")
                            );

                            $(".list-gallery-media-images").each(
                                (index, item) => {
                                    let $current = $(item);
        ...
        $(document).on(
            ...
            (event) => {
                event.preventDefault();
                $("#update-product-variation-button").data(
                    "target",
                ...
                success: (res) => {
                    if (res.error) {
                        ...
                        if ($table.find("tbody tr").length === 1) {
                            ...
                            $table
                                .find("#variation-id-" + $current.data("id"))
                                .fadeOut(400)
                                .remove();
                        }
...
$(window).on("load", () => {
    ...
    $(document).on("change", ".product-select-attribute-item", () => {
        ...
        if (selectedItems.length) {
            $(".btn-trigger-add-attribute-to-simple-product").removeClass(
                "hidden"
            );
        } else {
            $(".btn-trigger-add-attribute-to-simple-product").addClass(
                "hidden"
            );
        }
    });
    ...
    $(document).on("change", ".product-select-attribute-item", (event) => {
        ...
        $(event.currentTarget)
            ...
            .prop(
                "name",
                "added_attributes[" + $(event.currentTarget).val() + "]"
            );
    ...
    $(document).on("click", ".btn-trigger-add-attribute-item", (event) => {
        ...
        $.each(
            ...
            (index, el) => {
                if (
                    $(el).prop("value") !== $(el).closest("select").val() &&
                    $(el).prop("disabled") === false
                ) {
                    $template
                        .find(".product-select-attribute-item-value-wrap")
        ...
        $listDetailWrap
            ...
            .val(selectedValue);
        $listDetailWrap
            .find(".product-select-attribute-item-value-id-" + selectedValue)
            .prop("name", "added_attributes[" + selectedValue + "]");

        if (
            $listDetailWrap.find(".product-attribute-set-item").length ===
            $(
    ...
    $(document).on("click", ".product-set-item-delete-action a", (event) => {
        ...
        let $listProductAttributeWrap = $(
            ".list-product-attribute-wrap-detail"
        );
        if (
            $listProductAttributeWrap.find(".product-attribute-set-item")
                .length < 2
        ) {
                ...
                onSelectFiles: (files, $el) => {
                    ...
                    _.forEach(files, (file, index) => {
                        ...
                        let template = $(document)
                            .find("#product_select_image_template")
                            .html();
    ...
    $(document).on("click", ".btn-trigger-remove-product-image", (event) => {
        ...
        if (
            $(".list-gallery-media-images").find(".product-image-item-handler")
                .length === 0
        ) {
    ...
    $(document).on("click", ".box-search-advance .page-link", (event) => {
        ...
        if (
            !$searchBox.closest(".page-item").hasClass("disabled") &&
            $searchBox.data("target")
        ) {
            let $formBody = $searchBox
                .closest(".box-search-advance")
                .find(".panel");
            Cmat.blockUI({
                target: $formBody,
    ...
    function humanFileSize(bytes, si = true, dp = 2) {
        ...
        return Cmat.numberFormat(bytes, dp, ",", ".") + " " + units[u];
    }
});
