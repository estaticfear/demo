<template>
    <div>
        <div class="">
            <h6 class="text-center">Hiện vật</h6>
        ...
        <div class="col-12 col-sm-6 col-md-12 col-lg-6 text-end d-flex">
            <button type="button" class="btn btn-primary" v-b-modal.update-physical-products>
                {{ __('Cập nhật') }}
            </button>
        </div>
        <b-modal
            id="update-physical-products"
            :title="__('Xác nhận cập nhật')"
            :ok-title="__('Cập nhật')"
            :cancel-title="__('Đóng')"
            @ok="updateProjectProducts($event)"
        >
            <div class="note note-warning">{{ __('Bạn chắc chắn muốn cập nhật danh sách hiện vật chứ?') }}.</div>
        </b-modal>
    </div>
</template>
            ...
            axios
                ...
                .then((res) => {
                    ...
                    if (res.data.error) {
                        ...
                        context.$root.$emit('bv::hide::modal', 'update-physical-products');
                        setTimeout(() => {
