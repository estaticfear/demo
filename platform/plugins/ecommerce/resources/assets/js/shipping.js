    ...
    init() {
        $(document).on(
            ...
            (event) => {
                ...
                $modal
                    .find("#confirm-delete-region-item-button")
        ...
        $(document).on(
            ...
            (event) => {
                ...
                $.ajax({
                    type: "POST",
                    url: $("div[data-delete-region-item-url]").data(
                        "delete-region-item-url"
                    ),
                    data: {
                        _method: "DELETE",
                        id: _self.data("id"),
                    ...
                    success: (res) => {
                        if (!res.error) {
                            $(
                                ".wrap-table-shipping-" + _self.data("id")
                            ).remove();
        ...
        $(document).on(
            ...
            (event) => {
                ...
                $modal
                    ...
                    .text($(event.currentTarget).data("name"));
                $modal
                    .find("#confirm-delete-price-item-button")
                    .data("id", $(event.currentTarget).data("id"));
                $modal.modal("show");
            }
        );
        ...
        $(document).on(
            ...
            (event) => {
                ...
                $.ajax({
                    type: "POST",
                    url: $("div[data-delete-rule-item-url]").data(
                        "delete-rule-item-url"
                    ),
                    data: {
                        _method: "DELETE",
                        id: _self.data("id"),
                    ...
                    success: (res) => {
                        if (!res.error) {
                            $(
                                ".box-table-shipping-item-" + _self.data("id")
                            ).remove();
                            if (res.data.count === 0) {
                                $(
                                    ".wrap-table-shipping-" +
                                        res.data.shipping_id
                                ).remove();
                            }
                            Cmat.showSuccess(res.message);
                        } else {
                ...
                success: (res) => {
                    if (!res.error) {
                        ...
                        if (res?.data?.rule?.shipping_id && res?.data?.html) {
                            const $box = $(
                                ".wrap-table-shipping-" +
                                    res.data.rule.shipping_id +
                                    " .pd-all-20.border-bottom"
                            );
                            const $item = $box.find(
                                ".box-table-shipping-item-" + res.data.rule.id
                            );
        ...
        $(document).on("keyup", ".input-sync-item", (event) => {
            ...
            $this
                ...
                .text(Cmat.numberFormat(parseFloat(number), 2));
        });
        ...
        $(document).on("click", ".btn-add-shipping-rule-trigger", (event) => {
            ...
            $("#add-shipping-rule-item-button").data(
                "shipping-id",
                $this.data("shipping-id")
            );
            $modal
                .find("select[name=type] option[disabled]")
                .prop("disabled", false);
            if (!$this.data("country")) {
                $modal
                    .find("select[name=type] option[value=base_on_zip_code]")
                    .prop("disabled", true);
            }
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        if (!res.error) {
                            const $table = $(
                                ".table-shipping-rule-" +
                                    res.data.shipping_rule_id
                            );
                            if (
                                $table.find(
                                    ".shipping-rule-item-" + res.data.id
                                ).length
                            ) {
                                $table
                                    .find(".shipping-rule-item-" + res.data.id)
        ...
        $(document).on(
            ...
            (event) => {
                ...
                $modal
                    ...
                    .text($(event.currentTarget).data("name"));
                $modal
                    .find("#confirm-delete-shipping-rule-item-button")
                    .data("url", $(event.currentTarget).data("section"));
                $modal.modal("show");
            }
        );
            ...
            (event) => {
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        ...
                        $("#confirm-delete-shipping-rule-item-modal").modal(
                            "hide"
                        );
                    },
                    error: (error) => {
                ...
                success: (res) => {
                    if (!res.error) {
                        ...
                        $(".wrapper-content").load(
                            window.location.href + " .wrapper-content > *"
                        );
                    } else {
                        Cmat.showError(res.message);
                    }
        ...
        $(document).on("keyup", ".base-price-rule-item", (event) => {
            ...
            $.each(
                ...
                (index, item) => {
                    let adjustmentPrice = $(item)
                        .closest("tr")
                        .find(".shipping-price-district")
                        .val();
                    ...
                    $(item).text(
                        Cmat.numberFormat(
                            parseFloat(basePrice) + parseFloat(adjustmentPrice)
                        ),
                        2
                    );
                }
