...
class Footprinter implements FootprinterInterface
{
    ...
    public function footprint(Request $request): string
    {
        ...
        if ($request->hasCookie('cmat_footprints_cookie')) {
            return $request->cookie('cmat_footprints_cookie');
        }

        // This will add the cookie to the response
        Cookie::queue(
            'cmat_footprints_cookie',
    ...
    {
        $data = Cookie::get('cmat_footprints_cookie_data');
